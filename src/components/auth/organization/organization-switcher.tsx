...
import { type ComponentProps, type ReactElement, useState, useEffect } from "react"
...
export function OrganizationSwitcher({
  ...
  // Auto-activate last used or first organization if activeOrganization is null
  useEffect(() => {
    if (
      session?.user &&
      ...
      if (targetOrg?.id) {
        setActiveOrganization({ organizationId: targetOrg.id })
  ...
  function handleSetActive(organization: Organization | null) {
    ...
    if (organization) {
      if (typeof window !== "undefined") {
        localStorage.setItem("better-auth.activeOrgId", organization.id)
      }
      setActiveOrganization({ organizationId: organization.id })
      if (organization.slug) {
        navigate({ to: `/dashboard/${organization.slug}` })
      }
    } else if (setActive) {
      setActive(organization)
    } else {
      setActiveOrganization({ organizationId: null })
    }
  }
